...
import static ch.uzh.se.se7en.junit.server.TestUtil.verifyPersisted;
...
public class TriggerImportServiceTest {
	...
	public void setup() {
		...
		// mock the queries for fetching all the existing countries, genres,
		// languages
	...
	public void testImportFilmsToDB() {
		...
		assertEquals(importService.getCountryMap().size(), 12); // 2 existing +
																// 10 added
		assertEquals(importService.getGenreMap().size(), 8); // 3 existing + 5
																// added
		assertEquals(importService.getLanguageMap().size(), 4); // 1 existing +
																// 3 added
